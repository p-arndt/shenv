// Package update handles self-updating the shenv binary from GitHub Releases and
// the lightweight "a newer version is available" notice.
//
// The release pipeline publishes one archive per platform named
// shenv_<version>_<goos>_<goarch>.{tar.gz,zip} plus a checksums file (see
...
// truncation: installing a truncated binary would brick the user's shenv.
...
// because versions get printed to the user's terminal, cached in
// ~/.shenv/update-check.json, and interpolated into asset file names.
...
func (c *Client) LatestRelease(ctx context.Context) (*Release, error) {
	...
	req.Header.Set("User-Agent", "shenv-updater")
...
func ArchiveName(version, goos, goarch string) string {
	...
	return fmt.Sprintf("shenv_%s_%s_%s.%s", version, goos, goarch, ext)
}

// ChecksumsName is the checksums file name for a version.
func ChecksumsName(version string) string {
	return fmt.Sprintf("shenv_%s_checksums.txt", version)
}

// findAsset returns the asset with the given name, or an error naming what was
// available (helps diagnose a platform the release didn't build for).
...
// run (see CleanupLeftovers).
func replaceExecutable(exePath string, newBin []byte) error {
	dir := filepath.Dir(exePath)
	tmp, err := os.CreateTemp(dir, ".shenv-update-*")
	if err != nil {
		return fmt.Errorf("cannot write to %s (%w) — reinstall shenv or run with sufficient permissions", dir, err)
	}
	tmpName := tmp.Name()
	...
	if runtime.GOOS == "windows" {
		...
		if err := os.Rename(exePath, old); err != nil {
			return fmt.Errorf("cannot move current binary aside (%w) — is another shenv running?", err)
		}
		...
		return fmt.Errorf("cannot replace %s (%w) — reinstall shenv or run with sufficient permissions", exePath, err)
	}
